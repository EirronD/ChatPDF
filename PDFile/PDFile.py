...
def change_user_password_in_firestore(email, new_password):
    try:
        ...
        print(f"Error changing password in Firestore: {e}")
        return False
...
def forgot_password():
    if request.method == 'POST':
        ...
        if 'otp' in session and session['otp_verified']:
            try:
                success = change_user_password_in_firestore(email, new_password)
                if success:
                    session.pop('otp', None)
                    session.pop('otp_verified', None)
                    return redirect(url_for('login'))
                else:
                    return "Error changing password in Firestore: User not found", 500
            except Exception as e:
                return f"Error: {e}", 500
        else:
            return "OTP not verified", 403
    else:
        return render_template('forgotpass.html', error=None)
